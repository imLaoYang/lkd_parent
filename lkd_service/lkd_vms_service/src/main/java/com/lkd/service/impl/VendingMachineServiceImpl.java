...
//    private Sender sender;
  ...
  public boolean supply(SupplyCfg supply) {
    ...
    supply.getSupplyData().forEach(supplyData -> {
              Optional<ChannelEntity> item = channelList.stream()
                              .filter(channel -> channel.getChannelCode().equals(supplyData.getChannelId()))
                              .findFirst();
              if (item.isPresent()) {
                ChannelEntity channelEntity = item.get();
                channelEntity.setCurrentCapacity(channelEntity.getCurrentCapacity() + supplyData.getCapacity());
